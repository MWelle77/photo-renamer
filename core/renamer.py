...
def _find_free_name(directory: Path, stem: str, suffix: str, reserved: set) -> str:
...
def plan_renames(records: List[FileRecord]) -> List[Tuple[Path, str]]:
    ...
    """
    # Group by directory so collision tracking is per-folder
    ...
    for directory, recs in by_dir.items():
        ...
        # Sort deterministically so collision numbering is stable across runs
        recs_sorted = sorted(recs, key=lambda r: (r.path.name,))

        for rec in recs_sorted:
            stem = build_target_stem(rec.dt, rec.device)
            suffix = rec.path.suffix.lower()
            new_name = _find_free_name(directory, stem, suffix, reserved)
...
def execute_renames(
    ...
    for idx, (src, new_name) in enumerate(plan):
        ...
        # If dest exists and is not src, it was not caught by plan_renames
        # (e.g. added between plan and execute) - skip safely
        ...
        try:
            ...
            # Rename companion XMP sidecar if it exists
            xmp_src = src.with_suffix('.xmp')
            if not xmp_src.exists():
                xmp_src = src.with_suffix('.XMP')
            if xmp_src.exists():
                xmp_dest = dest.with_suffix('.xmp')
                if not xmp_dest.exists():
                    xmp_src.rename(xmp_dest)
        except OSError as e:
